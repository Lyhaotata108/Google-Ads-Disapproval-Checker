(() => {
  ...
  function addStyles() {
    if (document.getElementById("preflight-v4-css")) return;
    const style = document.createElement("style");
    style.id = "preflight-v4-css";
    style.textContent = `
  ...
  function bindForm() {
    ...
    if (!form || form.dataset.preflightV4Bound) return;
    form.dataset.preflightV4Bound = "1";
  ...
  function calculate() {
    ...
    const hasAi = Boolean(analysisData && Number.isFinite(Number(analysisData.complianceScore)));
  ...
  function renderPrediction(force = false) {
    ...
    if (running && !analysisData && !forensicsData) {
      html = `<div class="pf-loading">正在生成投放前拒登预判：等待页面政策分析与技术取证结果...</div>`;
    } else if (!analysisData && !forensicsData) {
      card.remove();
      lastRenderSignature = "";
      return;
    } else {
  ...
  function maintainUi() {
    ensureInput();
    bindForm();
    if (analysisData || forensicsData || running) {
